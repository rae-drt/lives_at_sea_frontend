...
import { officerref, RATING_LAYOUT, OFFICER_LAYOUT } from './data_utils';
...
export default function Person() {
  const { sailorType, piece, item, dataType } = useParams();
  const { pathname } = useLocation();
  const navigate = useNavigate();
  const { data: personTableData, setData: setPersonTableData, mutateData: mutatePersonTableData, status: mainPersonQueryStatus } = useRecord(sailorType, piece, item, 'name');
  const dirty = useDirtySailor(sailorType, piece, item);
  const blocker = useDirtySailorBlocker(dirty);

  if(isNew(piece, item)) document.title = 'New ' + sailorType;
  else if(personTableData) {
    if(sailorType === 'rating') document.title = `Rating ADM 188/${piece}/${item}`;
  ...
  else document.title = 'Fetching ' + sailorType + ' ADM 188/' + piece + '/' + item;
  ...
  if(
    ...
    const controlPanel = isNew(piece, item) ?
          ...
          <Stack direction='row' spacing={2} alignItems='center' justifyContent='space-around' width={0.95}>
            <Stack sx={{alignItems: 'center', justifyContent: 'space-evenly'}} spacing={4}>
              ...
              <Stack alignItems='center' spacing={2} width='95vw'>
                <Tabs value={dataType} onChange={(e,v) => {navigate('/' + sailorType + '/' + piece + '/' + v);}}>
